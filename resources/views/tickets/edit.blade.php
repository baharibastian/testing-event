            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        ...
                        <div class="body">
                            <form role="form" method="post" action="{{ url('user/ticket/update', $ticket->ticket_id) }}">
                                {{ csrf_field() }}
                                <label for="ticket_name">Nama Tiket</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="text" id="ticket_name" name="ticket_name" value="{{ $ticket->ticket_name }}" class="form-control" placeholder="Masukkan Nama Acara">
                                    </div>
                                </div>
                                <label for="ticket_price">Harga Tiket</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="text" id="ticket_price" name="ticket_price" value="{{ $ticket->ticket_price }}" class="form-control" placeholder="Masukkan Deskripsi Acara">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <?php $selected = '' ?>
                                    <select name="event_id">
                                        @foreach($events as $data)
                                            <?php 
                                            if ($ticket->event_id == $data->event_id) $selected = "selected";
                                            else $selected = '';
                                            ?>

                                            <option value="{{ $data->event_id }}" <?php echo $selected ?>>{{ $data->event_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
